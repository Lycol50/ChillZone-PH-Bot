    ...
    try {
        ...
        for (let i = 0; i < queue.songs.length; i += 10) {
            ...
            const embed = new Discord.MessageEmbed()
                ...
                .setURL(qus[0].url)
                .setColor("RANDOM")
                .setDescription(`__Now Playing:__\n[${qus[0].name}](${qus[0].url}) | \`${qus[0].formattedDuration} Requested By: ${qus[0].user.tag}\`\n\n__Up Next:__\n${info}`)
                .setFooter(`${client.user.username} | Loop: ${queue.repeatMode ? queue.repeatMode == 2 ? "Queue" : "This Song" : "Off"}`  + ` | Filter: ${queue.filter || "❌"}`, client.user.displayAvatarURL())
            embeds.push(embed);
        }
        //returning the Embed
        return embeds;
